...
| Settings — provider enum | Frontend: `AiProvider = "openai" \| "openrouter" \| "anthropic" \| "custom"` | Backend: `[JsonStringEnumConverter]` → `"Unknown"`, `"OpenAI"`, `"OpenRouter"`, `"Gemini"` | ⚠️ Несовпадение | 1) Кейсинг: backend отдаёт `"OpenAI"`, frontend ожидает `"openai"`. 2) `"anthropic"` и `"custom"` нет в backend. 3) `"Gemini"` нет в frontend. Нужно согласовать enum |
...
| Удаление папки | Frontend: `deleteFolder(id, recursive)` шлёт `?recursive=true` | Backend: `DELETE /api/folders/{id}` **не принимает** параметр `recursive` (контроллер ещё мок) | ⚠️ Несовпадение | Backend должен добавить `[FromQuery] bool recursive = false` в `DeleteFolder` |
