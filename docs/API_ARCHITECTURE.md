...
|---|---|---|
| Упорядочивание папок | `Folder.SortOrder` | Требуется для операций переупорядочивания соседних элементов. |
| Конфигурация ИИ пользователя | `UserSettings` с зашифрованным API-ключом и `LastTestedAt` | Требуется для конфигурации LLM для каждого пользователя. |
