...
**AiSettingsDto** включает `provider` (enum AiProvider: OpenAI, OpenRouter, Gemini, Anthropic) и `modelId` (string?) наряду с существующими base URL и API key, позволяя пользователям выбирать конкретных LLM-провайдеров и модели. Backend сериализует enum-значения в lowercase и парсит case-insensitively.
