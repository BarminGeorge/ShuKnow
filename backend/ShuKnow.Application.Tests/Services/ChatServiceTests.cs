...
public class ChatServiceTests
{
    ...
    [Test]
    public async Task CreateSessionAsync_WhenCalled_ShouldCreateAndPersistSession()
    {
    ...
    [Test]
    public async Task DeleteSessionAsync_WhenSessionDoesNotExist_ShouldReturnNotFoundWithoutPersistingChanges()
    {
        var result = await sut.DeleteSessionAsync(Guid.NewGuid());
    ...
    [Test]
    public async Task DeleteSessionAsync_WhenSessionExists_ShouldDeleteMessagesAndSession()
    {
        var session = CreateSession();
        chatSessionRepository.GetByIdAsync(session.Id, currentUserId).Returns(Success(session));

        var result = await sut.DeleteSessionAsync(session.Id);
    ...
    {
        var result = await sut.GetMessagesAsync(Guid.NewGuid(), cursor: null, limit: 0);

        result.Status.Should().Be(ResultStatus.Error);
        await chatSessionRepository.DidNotReceive().GetByIdAsync(Arg.Any<Guid>(), Arg.Any<Guid>());
        await chatMessageRepository.DidNotReceive().GetPageAsync(Arg.Any<Guid>(), Arg.Any<string?>(), Arg.Any<int>());
    }

    [Test]
    public async Task GetMessagesAsync_WhenCalled_ShouldReturnRepositoryPageForSession()
    {
        var session = CreateSession();
        ...
        var result = await sut.GetMessagesAsync(session.Id, cursor, 25);
    ...
    public async Task GetMessagesAsync_WithoutCursor_ShouldReturnSortedMessagesFromRepository()
    {
        ...
        var result = await sut.GetMessagesAsync(session.Id);
    ...
    [Test]
    public async Task GetMessageCountAsync_WhenCalled_ShouldReturnRepositoryCountForSession()
    {
        var session = CreateSession();
        ...
        var result = await sut.GetMessageCountAsync(session.Id);
    ...
    private void ConfigureDefaults()
    {
        ...
        chatSessionRepository.GetByIdAsync(Arg.Any<Guid>(), Arg.Any<Guid>()).Returns(NotFound<ChatSession>());
